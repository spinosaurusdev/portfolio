...
function getLocation() {
    if (navigator.geolocation) {
        ...
        document.getElementById('clima').textContent = "Geolocation is not supported by this browser.";
    }
}
...
function handleError(error) {
    switch(error.code) {
        case error.PERMISSION_DENIED:
            document.getElementById('clima').textContent = "The user denied the geolocation request.";
            break;
        case error.POSITION_UNAVAILABLE:
            document.getElementById('clima').textContent = "Location information is not available.";
            break;
        case error.TIMEOUT:
            document.getElementById('clima').textContent = "The request to obtain the user's location has expired.";
            break;
        default:
            document.getElementById('clima').textContent = "An unknown error occurred.";
            break;
    }
}
...
function getClima(position) {
    ...
    fetch(url)
        ...
        .then(data => {
            ...
            document.getElementById('clima').textContent = `In ${city}, the climate is ${description} with a temperature of ${temperature}°C.`;
        })
        .catch(error => {
...
//Script para el efecto de sombra dinámica en el título principal
        ...
        function updateShadow(event) {
            ...
            const shadowX = deltaX * 0.1; // Multiplicador para suavizar el movimiento
            const shadowY = deltaY * 0.1;

            // Aplicar la sombra dinámica
            shadow.style.textShadow = `${shadowX}px ${shadowY}px 14px var(--background)`;
        }
